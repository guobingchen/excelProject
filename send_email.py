...
def import_excel():  # 导入Excel文件
    ...
    if not filename:
        output_text.insert(tk.END, "请选择一个Excel文件\n")
        return
    ...
    try:
        ...
        for name, group in grouped:
            ...
            for index, row in group.iterrows():
                content += ("[" + str(row["课程名称"]) + "]" + ":" + "[" + str(row["百分成绩"]) + "]" + "[" + str(
                    row["五分成绩"]) + "]" + "[" + str(row["考试类型"]) + "]" + "[" + str(row["选修类型"]) + "]" + "\n")
        ...
        output_text.insert(tk.END, "文件不存在或路径错误\n")
    except Exception as e:
        output_text.insert(tk.END, "发生了错误: " + str(e) + "\n")
...
def send_email():
    ...
    receiver = receiver_entry.get()  # 获取输入框中的邮箱地址
    if not receiver:
        output_text.insert(tk.END, "请输入接收者邮箱地址\n")
        return
    ...
    try:
        ...
        output_text.insert(tk.END, f"成绩单 {index + 1} 已成功发送至 {receiver}\n")
        index += 1
    except Exception as e:
